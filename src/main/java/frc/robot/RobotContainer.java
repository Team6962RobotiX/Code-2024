...
public class RobotContainer {
  ...
  public RobotContainer() {
    ...
    if (!EnabledSystems.ENABLE_DRIVE) {
      scheduler.unregisterSubsystem(drive);
    }
    if (!EnabledSystems.ENABLE_DASHBOARD) {
      scheduler.unregisterSubsystem(dashboard);
    }
    if (!EnabledSystems.ENABLE_LIMELIGHT) {
      scheduler.unregisterSubsystem(limelight);
    }
    if (EnabledSystems.ENABLE_TESTING) {
      scheduler.unregisterSubsystem(drive);
      scheduler.unregisterSubsystem(dashboard);
      scheduler.unregisterSubsystem(limelight);
    }
