 ...
 * CustomPoseEstimator#addVisionMeasurement} and only call {@link CustomPoseEstimator#update} then this will
 * behave exactly the same as Odometry.
 *
 * <p>{@link CustomPoseEstimator#update} should be called every robot loop.
 *
 * <p>{@link CustomPoseEstimator#addVisionMeasurement} can be called as infrequently as you want; if you
...
public class CustomPoseEstimator<T extends WheelPositions<T>> {
   ...
   * CustomPoseEstimator#update} every loop.
   *
   ...
   *     CustomPoseEstimator#updateWithTime(double,Rotation2d,WheelPositions)} then you must use a
   ...
   * CustomPoseEstimator#setVisionMeasurementStdDevs(Matrix)} or this method.
   *
   * @param visionRobotPoseMeters The pose of the robot as measured by the vision camera.
   * @param timestampSeconds The timestamp of the vision measurement in seconds. Note that if you
  ...
  private class InterpolationRecord implements Interpolatable<InterpolationRecord> {
    ...
    public boolean equals(Object obj) {
      ...
      if (!(obj instanceof CustomPoseEstimator.InterpolationRecord)) {
        return false;
      }
      var record = (CustomPoseEstimator<?>.InterpolationRecord) obj;
